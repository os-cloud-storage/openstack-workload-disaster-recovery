...
class DrbdManageBaseDriver(driver.ISCSIDriver):
    ...
    def _priv_hash_from_volume(self, volume):
        return dm_utils.dict_to_aux_props({
            AUX_PROP_CINDER_VOL_ID: volume.name_id,
        })
    ...
    def create_volume(self, volume):
        ...
        d_res_name = self.is_clean_volume_name(volume.name_id, DM_VN_PREFIX)
        ...
        if not okay:
            message = (_('DRBDmanage timeout waiting for volume creation; '
                       ...
                       {'res': d_res_name, 'vol': volume.name_id})
            raise exception.VolumeBackendAPIException(data=message)
    ...
    def manage_existing_get_size(self, volume, existing_ref):
        ...
        try:
          (r_name, v_nr, r_props, v_props) = self._res_and_vl_data_for_volume(lv_id)
        except:
          kwargs = {'existing_ref': lv_id,
    ...
    def create_volume_from_snapshot(self, volume, snapshot):
        ...
        LOG.debug("create vol from snap: from %(snap)s make %(vol)s",
                  {'snap': snapshot['id'], 'vol': volume.name_id})
        # TODO(PM): Consistency groups.
        d_res_name, sname, sprop = self._resource_and_snap_data_from_snapshot(
            snapshot)

        new_res = self.is_clean_volume_name(volume.name_id, DM_VN_PREFIX)
...
class DrbdManageIscsiDriver(DrbdManageBaseDriver):
    ...
    def create_export(self, context, volume, vg=None):
        volume_path = self.local_path(volume)
        LOG.info("volume_path %s" % volume_path)
        LOG.info("configuration %s" % self.configuration)
        LOG.info("volume name is method %s" % (self.is_clean_volume_name(volume['name'], DM_VN_PREFIX)))
        LOG.info("volume_path %s" % volume_path)

        LOG.info("provider_auth %s iscsi_target %s" % (volume['provider_auth'],
                                                       volume['iscsi_target']))
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        return None
        #return self.target_helper.terminate_connection(volume,
        #                                               connector,
        #                                               **kwargs)
