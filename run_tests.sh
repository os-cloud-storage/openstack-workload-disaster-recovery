...
function usage {
 "Usage: $0 [OPTION]..."
echo "Run dragon's test suite(s)"
echo ""
...
# provided.
testrargs="discover ./dragon/tests"
fi
...
if [ setup.cfg -nt dragon.egg-info/entry_points.txt ]
then
...
${wrapper} coverage html --include='dragon/*' --omit='dragon/openstack/common/*' -d covhtml -i
fi
return $RESULT
}
...
bash -c "${wrapper} flake8 dragon* bin/*"
}
